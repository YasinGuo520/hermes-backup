...
> 吸收自 `skill-creator`（curator 合并）——与父技能的“搜索/安装 Skill”配套：这里是写自己的 Skill，优先扩展现有技能而不是造窄兄弟。
...
## Frontmatter 硬性要求（源：`tools/skill_manager_tool.py::_validate_frontmatter`）

- 文件**第一个字节就是 `---`**（前面不能有空行/BOM），以 `\n---\n` 闭合
- 合法 YAML mapping；`name` 必填（小写+连字符，≤64 chars）；`description` 必填（≤1024 chars）
- 正文非空
- ⚠️ **新建 skill 的 description 必须 ≤60 chars（一句话、触发词在前、句号结尾）**——索引会把长描述截到 57 字，路由信号就废了；细节搬进正文
- 建议补齐：`version` / `author` / `license: MIT` / `metadata.hermes.{tags, related_skills}`
...
- description ≤1024 chars（强制）；前 57 chars 进系统提示词索引
- 整个 SKILL.md ≤100,000 chars（强制，~36k tokens）
- 优秀体积 8-14k chars；超 20k 就把分支细节拆进 `references/*.md`
...
2. **控制上下文负担**：description 每 turn 都付费，聚焦触发类，细节进正文/references。
3. **信息分层**：常用步骤进 SKILL.md；分支/大块资料进 `references/`、`templates/`、`scripts/`。
4. **步骤带完成标准**：每步说清怎么算做完（可检查、尽量穷尽）。
5. **规则就近存放**：定义、坑、示例、验证放一起。
6. **用强引导词**：用模型已懂的概念（"tight loop"、"root cause"）省 token。
7. **去重**：同一含义只留一个出处。
8. **防提前完成**：Agent 容易跳过的步骤，先强化该步的完成标准。

常见质量失败：**提前完成** / **重复**（同规则多处出现后漂移）/ **沉积**（只加不删的旧行）/ **臃肿**（常显内容过多）/ **无操作散文**（“要小心”→改成可检查的完成标准）。
...
4. **本地校验**：
   ...
   ```
5. **git add + commit**
6. **注意**：当前会话的 skill 加载器有缓存——新 skill 要新会话才可见

## 编辑现有 Skill

- 小改：`skill_manage(action='patch', name=..., old_string=..., new_string=...)`
- 大改：write_file 整个 SKILL.md，或 `skill_manage(action='edit')`
- 加支持文件：`skill_manage(action='write_file', file_path='references/<x>.md')`（限 references/templates/scripts/assets 子目录；**bundled 内置技能拒写**——只能整体归档，不能打补丁）
- ⚠️ 后台 curator 对 SKILL.md 的 patch 有 read-before-write 门禁：需要同一轮内先 `skill_view` 拿到内容再写；若门禁拒写（去重导致内容不回），改用 `write_file` 把内容落成 `references/*.md`

## Common Pitfalls

1. **in-repo 用 `skill_manage(action='create')`** → 写到了 `~/.hermes/skills/` 而不是 repo 树。用 write_file。
2. **`---` 前有空白/BOM** → 校验失败。
3. **description 太泛 / 触发词埋在 57 字之后 / 新建时超 60 字** → 索引截断丢路由信号。
4. **缺 author/license/metadata** → 不强制但显得半成品。
5. **重复造轮子** → 创建前先 `ls skills/<category>/` 看同行，优先扩展现有 skill 而不是造窄兄弟。
6. **指望当前会话立刻看到新 skill** → 加载器会话开始时初始化。
7. **沉积** → skill 应该越改越短/越准。
8. **无操作散文** → 换成可检查的完成标准。
9. **related_skills 引用仅用户级存在的 skill** → 别人 clone repo 后失效；in-repo 只链 in-repo。

## Verification Checklist

- [ ] 文件位置正确（用户级 `~/.hermes/skills/` 或 in-repo `skills/<category>/`）
- [ ] frontmatter 从字节 0 开始 `---`，`\n---\n` 闭合
- [ ] name（≤64 小写连字符）/ description（新建 ≤60 chars，触发词在最前）
- [ ] 总长 ≤100k chars（目标 8-14k）
- [ ] 结构：Overview → When to Use → 正文 → Pitfalls → Verification Checklist
- [ ] 每步有可检查的完成标准；大块内容已拆进 references/templates/scripts
- [ ] 无 no-op 散文、无重复规则
