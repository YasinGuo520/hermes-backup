...
Qwen-Image text-to-image **无法保持跨图的人物/产品一致性**。每张图独立生成，以下现象是正常的：

- 同一个 prompt 出两张图，人物长相不同 ✅ 正常
- 产品上的 logo/文字每次位置不同 ✅ 正常
- 材质/颜色有偏差 ✅ 正常

**根本原因：** 无参考图机制、无记忆、Prompt 语意损耗。详见 `references/consistency-zh.md`。

### 用户问"为什么不能保持一致"时直接回答

不要找借口，直接说核心原因：
1. Qwen-Image 没有 Character/IP Adapter 能力
2. 每次都是独立掷骰子
3. 要给一致性需要换方案（见 reference）
...
|--------|---------------|----------|
| 单张产品图 | ✅ 直接用 | — |
| 多角度同一产品 | ⚠️ 去背景+image-to-image，效果有限 | ComfyUI+ControlNet |
| 同一人物在多张图 | ❌ 不要硬出 | ComfyUI+IP-Adapter/InstantID |
| 人物+产品都保一致 | ❌ 不要硬出 | 视频管线（Wan2.2）或 ComfyUI 全套 |
