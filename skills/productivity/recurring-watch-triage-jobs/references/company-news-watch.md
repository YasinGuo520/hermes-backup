...
Use feeds and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to `~/.hermes/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the recurring-watch-triage-jobs skill, read references/company-news-watch.md, and run the tick for the watch contract at ~/.hermes/competitor-watches/<watch-slug>.json.",
