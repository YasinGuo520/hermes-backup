...
**权威规则不在此重复**：五条硬过滤（今天能准备 / 7天能收钱 / 0资金投入 / 单人交付 / 有现成资源）与变现排序（卖服务 > 卖现成产品 > 按单接活 > 代理分销 > ❌做新SaaS）以 `monetization-case-daily-pipeline` 的「快速变现/接单/日结渠道库」为准。
**先调研后建议铁律**：所有平台建议先搜 `平台名 + 项目名 + 真实收入` 验证实际状态再给结论（多次犯过：咸鱼文书代写没销量、猪八戒单是刷的、AI商品图平台在限制）。约束清单见 `references/niche-survival-mode-user-constraints.md`。
