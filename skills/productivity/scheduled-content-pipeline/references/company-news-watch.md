# Recurring Company / Competitor News Watch

> Absorbed from the bundled `competitor-news-monitor` skill (curator consolidation). Setup runs once in the foreground; the recurring check runs as a `cronjob` tick.
...
Do not use for: one-off company research (use `web_search`/`web_extract` directly) or plain feed reading.
...
Use feeds and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `~/.hermes/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the scheduled-content-pipeline skill, read references/company-news-watch.md, and run the tick for the watch contract at ~/.hermes/competitor-watches/<watch-slug>.json.",
