...
## 十九、把 X 封装成「安装程序」卖？先过三道闸

**触发**：「把这个页面项目跟 X 一起封成安装程序卖行不行」「有没有市场」。

### 闸1：上游是不是已经免费提供了？（最常被忽略，一票否决）

先查上游本身。**上游已出官方安装包 → 封装这个动作归零**，客户自己去官网下载就行，你只多背一层维护责任。

| 查什么 | 怎么查 | 2026-09 实测（Hermes） |
|:--|:--|:--|
| 官方有没有桌面版/安装包 | 官网 `/desktop` 页 + 仓库 README 的 Quick Install 段 | **已有** `Hermes-Setup.dmg` / `Hermes-Setup.exe`，免费、不需账号 |
| 许可证能不能转售 | `read_file` 上游 `LICENSE` + 看 `pyproject.toml` 的 `license` 字段 | **MIT**，明文授权 "sublicense, and/or sell"；条件=分发时附版权声明+许可证全文 |
| 商标能不能用 | MIT 不含商标授权 | 不能挂 Nous Research / Hermes 的品牌，改名可以 |

**结论模板**：`法律能卖，商业不能卖` —— 许可证允许，但官方免费安装包已存在，封装无价值。

⚠️ 注意区分：**封装「上游 + 你自己的 UI/skill 包」是另一件事**，价值不在安装程序，在 UI + skill + 场景。

### 闸2：市场真实价格带是多少？（别自己定价）

查第三方实际在卖什么价。2026 实测（完整数据+来源见 `references/agent-packaging-and-install-service-market.md`）：

| 形态 | 价格带 |
|:--|:--|
| 远程安装（基础，装好能跑就算完） | 30-100 元 |
| 远程+配置（接模型+基础 Skills） | 100-300 元 |
| 全家桶（+飞书/企微接入+VIP群） | 300-500 元 |
| 同城上门（含现场教学） | 500-1000 元 |
| 企业批量部署（多台+培训+支持） | 1000-7000 元 |

### 闸3：能不能卖成「服务」而不是「软件」？

一人公司铁律：**卖服务 > 卖软件**。软件要背分发/支付/客服/退款/版本维护；服务一次交付、不复购也认了。

| 形态 | 客单价 | 竞争 | 判定 |
|:--|:--|:--|:--|
| A 卖安装程序 | 30-100 | 上游免费做 | ❌ |
| B 纯代装服务 | 50-500 | 极卷（是地板，不是生意） | ⚠️ 现金流可用 |
| C **行业定制包 + 教会用** | **1000-3000** | 几乎无人做 | ✅ |

**C 的护城河 = 用户独有的、别人装机时装不出来的东西**：已跑通的 skill 包 + 行业经验 + 现成的集成参照系。定位话术：不是「帮你装软件」，是「给你配一个 ×× 行业的 AI 员工」。

### 闸4（自建系统专用）：卖的是「系统」还是「用系统干活」？

**触发**：「把我建的这套 X（Agent 矩阵 / 驾驶舱 / 后台）完善一下卖给客户」。
2026-09 实测（16 Agent 矩阵 + 光点驾驶舱页）。用户认可这个判断，并据此改成「先做展示物再去接服务单」。

| 形态 | 卖什么 | 客单价 | 售后 | 客户感知 | 判定 |
|:--|:--|:--|:--|:--|:--|
| 卖系统给客户 | 一套要自己养的系统 | 1500-3000 一次性 | 无底洞（服务器/API Key/装环境/培训） | 「多了个要伺候的东西」 | ❌ |
| **用系统接活** | **结果**（诊断/陪跑/代运营） | **999-8000/月** | 可控、可复用 | 「我少请一个人」 | ✅ |

**核心句**：用户的这套系统是**产能**，不是**商品**。卖掉它 = 一次性收入 + 无限售后；用它干活 = 持续收入 + 客户只看结果。

**上会前必打的红方六条**（任一条答不上来就别做成产品，改做服务）：
1. **你自己都没用起来**——查数据库行数/最后写入时间（见下方 Pitfall），客户一问「看看你自己的数据」就穿帮
2. **卖「半自动化」，客户要的是「人省了」**——半自动=客户还得传 Excel、点按钮、自己判断，等于多养一个要伺候的系统
3. **数据源天生残缺**——如淘宝/拼多多/京东无公开 API（见「四、平台API对接指南」），而客户最想要的「多平台自动汇总」恰是拿不到的那一半
4. **交付成本**——每客户一台服务器谁出钱？托管就要多租户+隔离+备份（见「二、多租户隔离」）；装客户电脑则售后无底洞
5. **销售路径**——单客 1500-3000，月入 1 万要 3-7 个新客/月 + 每个都要演示交付培训；期间 0 收入
6. **这又是「再完善一轮」**——建系统是舒适区、卖东西不是（用户自省过「分析多执行少」，此时要点出来）

**正解**：系统当**销售武器**——演示「一个人 + 一张驾驶舱图 = 干了十几个部门的活」来卖服务，而不是把系统本身当货。

### 演示物（Demo Artifact）四要素

要把自建系统当展示物，四条缺一不可：

| # | 要素 | 检查 | 常见翻车 |
|:--|:--|:--|:--|
| 1 | **公网可达 + 脱端口号** | 逐个 `curl -s -o /dev/null -w "%{http_code}" http://<IP>:<port>/`；再确认有域名入口（`https://<子域>` 返回 200） | 只在本地跑 → 客户打不开；对外版本必须部署到公网机。**发 `IP:端口` 给客户 = 一眼看出是个人项目，专业度归零** → 演示物一律套域名，静态产物让 nginx 直出（0 进程）。流程见 `server-service-deployment` 的「给服务套上域名」一节（前端是 Next.js 的还要先过构建闸：`app/api/*` 会让静态导出直接失败；部署后做三层验收 = 验产物字符串 / 验线上资源 200 / 验保活能自己拉回）→ `server-service-deployment/references/nextjs-static-export-deploy-verification.md` |
| 2 | **状态是真的** | **停一个服务**，看 60s 内是否显示为离线 | 状态硬编码/纯装饰 → 客户问「这个亮着是啥意思」答不上 |
| 3 | **点得进去** | 每个入口点一遍，到真页面而非介绍卡 | 卡片只有文案、没有跳转出口 = 装饰画 |
| 4 | **有内容** | 每个页面点进去不是空表 | 空表 = 自曝弱点 → 灌样例数据并**标注「演示数据」**，不要冒充真实经营数据。灌入配方（**走各服务真实 API 而非手写 SQL** / 灌前快照+一键还原 / 底部横幅标注而非顶部 / **故意埋异常值让预警类功能在演示时有反应** / 逐个服务打列表 API 数条目做量化验收）→ `server-service-deployment/references/demo-data-seeding.md` |

**防呆**：不要为了让展示物「看起来完整」去建缺失的模块。缺的**如实标 standby / 待建**（灰、虚线、暗点）——客户看得出这是真系统而不是 PPT，**「待建」比「假装都有」更可信**；建新模块 = 又一个多星期 0 收入。

**交付标准一句话**：不是「所有模块都亮」，是**「发链接给客户，客户看得懂、点得进去、觉得这帮人有产能」**。

### 溢价点：安全加固

第三方调研原话：「安全加固是绝大多数代装师根本不做的」「**安全加固无价**」→ 带安全加固的代装 = 现成溢价点。
同一件事也是风险面：代装 = 你要操作客户电脑，权限等同「在客户电脑上做任何事」；客户也怕你植入后门。**进门先说清三件事**：客户自己输 API Key / 全程录屏 / 装完出安全检查清单。

---

### ⚠️ Pitfall：提议扩建、加固、写文档之前，先验证它到底在不在用

用户问「要不要优化 X」时，**先查使用数据**，别默认它在跑。

**2026-09 实测教训**：准备给用户的 16 个 Agent 矩阵补「禁区字段」，顺手一体检发现——16 个服务端口全部 listen，但 `company.db` 7 张表**总共 8 行数据**，最后写入是 10 天前，近一周零日志。**系统在空转，给它补文档 = 装修没人住的房子。** 差点交付一份废纸方案。

必查三项（一条都别省）：

```bash
ss -tlnp | grep -E ':(端口段)'                      # 服务在不在 listen（listen ≠ 在用）
ls -la --time-style=+%m-%d_%H:%M <db 文件>            # 数据最后修改时间
find <日志目录> -name '*.log' -newermt '<一周前>'      # 有没有真实请求
```

```python
# 表行数才是真相 —— 必须逐表 count(*) + 找时间字段 MAX()
import sqlite3
...
**判定**：行数少 + 最后写入久 + 无日志 = **空转**。此时正确答案是直接告诉用户「这东西没人用，先别装修」，而不是继续给方案。用户认可这个判断（回「先放下」）。
...
### 外部 Agent 平台拆解法（拆 → 映射 → 蒸馏）

**触发**：「X 是不是跟你一样的」「挖下它牛在哪」「我能学什么」。对象：Grok Bot / ChatGPT Work / Cursor / Claude Agents 等外部 Agent 产品。

**四步**：
1. **抓权威源** — 官方 `overview`/`security-approvals`/`skills-routines`/`computer-and-apps` 四页打底，再补 **1 篇实际使用者长文**（工作坊拆解/官方 PM 指南）。官方给设计意图，使用者长文给**真实瓶颈**。第三方转述的价格/平台支持一律标「以官方页为准」。
2. **落事实档案** — 每条事实带来源；官方自报数据必须标「团队自述、未经独立验证」。存 `~/Desktop/hermes/<产品>-distill/00-事实与来源.md`。
3. **映射自己（必须实测，凭印象=交付废纸）** — `grep -n -A5 approvals ~/.hermes/config.yaml`、数 cron、数 skills、查 Agent 矩阵有没有「禁区」字段。**如果要据此提议扩建/加固那个系统，先验证它在不在用（见下方 Pitfall）。**
4. **可蒸馏清单表** — `它的做法 | 我有什么 | 差距(✅/⚠️/❌) | 动作 | 成本`，最后挑 **3 条最高 ROI** 让用户拍板。

**回答格式（用户认可的形状）**：一句话结论 → 它牛在哪（带出处表）→ 我的现状体检（实测值）→ 最能抄的 3 条 → 反面教训（做平台时最值钱）→ 一个拍板点。问「是不是跟你一样的」时用对照表（维度 | 它 | 我），不要长篇方法论，差距要诚实标注别粉饰。

**首次拆解产出（Grok Bot，2026-09）**：角色卡写「禁区」、交接契约、人类闸门分级（`Require Approval` 优先于 `Always Allow`，规则必须窄）、示教学习、Skill/Routine 分离、排程三振法则、no-data 策略、**「共享电脑不是安全边界」**（角色提示词约束不了浏览器会话）。完整案例 + 自测代码 + 可复用三条见 `references/agent-platform-teardown.md`。

**用户状态判断**：「了解下/暂时不用」→ 只给概念+成本不 push；「帮我搭/想试试」→ 转执行模式；「给预算方案」→ 细化到具体人数+部门。

**JD→Agent落地拆解法**：用户丢来一堆岗位JD/职能清单（"统筹运营+AI/销售+AI/培训+AI…这些Agent怎么弄出来"）时——不要逐个造Agent。拆成「数据管道+LLM决策层+独立小件」，走 数据墙调查→可行档分级(🟢🟡🔴)→工具分工(Hermes写脚本为主/Dify只做RAG+对外演示/n8n暂缓)→三件套交付(管道+工作台页面挂Hub导航+飞书推送)。对外=自用+一层壳，先自用验证价值再上壳。完整方法+电商7职能案例+数据墙速查见 `references/agent-matrix-blueprint.md`。

**电商公司完整可自动化职能清单（7→16扩展，2026-09实测）**：前台（⑧内容生产/⑨合规审查/⑩舆情差评）、中台（⑪选品/⑫数据分析/⑬流程自动化）、后台（⑭供应链/⑮库存管控/⑯物流跟踪）、职能（⑰财务/⑱行政），加上原7个（统筹运营/销售分析/趋势监控/培训/招聘/绩效/流程）=16个。**零token四类**：库存预警/物流异常（纯规则API）、合规主体（规则库）、财务计算（脚本）——能脚本不走LLM。

**成本测算粒度（按agent逐项估算）**：月token = 频次 × 单次(输入+输出)token，再×单价（DeepSeek ¥1/M入、¥2/M出）。16个全开≈310万token/月≈**¥4-6/月**（④培训占大头138万，自用可砍半）；一次性开发160-260万token≈**¥3-5**。结论：费用不是瓶颈（年化¥50-80），瓶颈是数据源拍板和开发节奏。

**GitHub现成件侦察结论**：全链路成品**不存在**（要的商业不开源、要么21星MVP玩具）；单点有真件——`1688-cli`(供应链管道，AI可调/扫码登录，直接装)、`nexscope-ai/eCommerce-Skills`(157方法论技能包，选核心装)、`JC0v0/Customer-Agent`(拼多多WebSocket客服接入，参考补通道)；借鉴架构——`didilili/shopkeeper-agent`(NL2SQL问数)、`isunswang/tiktok-ecommerce-automation`(全链路编排蓝图)。库存/财务/招聘类通用件要接自建数仓，改造比自写贵，自写即最优。

**Hub导航挂载模式（自用工具墙）**：每个Agent = 数据管道 + 工作台页面 + 飞书推送，页面全部挂用户导航Hub。新增分类=一个 `<div class="section-title">⚙️ 分类名</div>` + `<div class="project-grid">` 内N张 `<a class="card">` 卡片（card-icon/card-name/card-desc/card-meta含port-badge+tags）。端口从已用最大+1顺序分配并登记避冲突。自用版页面=同一份数据的另一种展示，不增加LLM成本。批量改HTML/Obsidian文件用脚本读→替换→写回，比逐个patch可靠。落地清单明细见 `references/电商公司Agent矩阵_落地清单.md`。

**⚠️ 向现有HTML导航页插入卡片的铁律（2026-09实测翻车：插入16卡后用户报「跟中年人生重叠」）**：
1. **锚点必须包含前一张卡片的完整闭合标签**——用 `</a></div><div class="section-title">🆕 新项目</div>` 这种锚点时，新section插入后闭合要保留在原位；替换串里漏掉 `</a></div>` = 前一张卡的`<a>`标签悬空，把新分类**包进前一张卡片**（视觉上就是两个分类重叠/布局错乱）
2. **插入后必验证（脚本断言，不是目测）**：①卡片总数=预期（`html.count('class="card"')`）②无连续闭合 `</a></div></a></div>` ③无 `card-arrow` 后直接跟 `section-title`（这是悬空`<a>`的指纹）④每个 section-title 前有 project-grid 闭合
3. 插新分类时**先读目标文件的当前结构**（section顺序、卡片结尾长什么样），再写替换串——不要凭记忆里的旧结构写
4. 插完顺手更新页头统计（如 `28 个项目 · 17 在线 · 16 规划`），新页面未部署前标「规划」不标「在线」
