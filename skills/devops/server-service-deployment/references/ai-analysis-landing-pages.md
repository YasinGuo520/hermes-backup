...
判断工具箱里某个技能能不能做成「输入→AI出结论」落地页，四要两不要：

- ✅ **纯推理可完成**：不依赖爬虫/本地文件/外部实时数据，用户填一段文字就能跑
- ✅ **输入简单**：textarea 一段话即可，不需要上传文件
- ✅ **输出结构化**：能出 JSON 卡片/表格，不是文件操作（生成视频/图片/PPT 的不行）
- ✅ **高频刚需**：用户天天用，或对内容营销有引流价值
- ❌ **依赖工具链**：剪映/ffmpeg/文生图 API 类——落地页做不了文件操作
- ❌ **依赖实时数据**：A股行情/抖音爬虫类——纯 LLM 会编数据，除非页面接数据源

已筛出 13 个候选（三梯队+推荐建设顺序）见 `references/candidate-pages-backlog.md`。新页面开工前先查这个池子，避免重复评估。
...
- `references/server-pattern.py` — 红蓝分析法 server.py 完整骨架（四段式文本解析版，改提示词即用）
- `references/server-pattern.py` — 六分身 JSON 版骨架（response_format json_object）
